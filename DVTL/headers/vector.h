...
namespace DVTL 
{
	...
	private:
		...
		void CheckValidIter(const_iterator& position);
	...
	public:
		...
		iterator(iterator&& right)						:ptr(right.ptr)	{ right.ptr = nullptr; }
		iterator& operator=(const iterator& right)	{ ptr = right.ptr; return *this; }
		iterator& operator=(iterator right)			{ ptr = right.ptr; right.ptr = nullptr; return *this; }
		~iterator() {}
	...
	public:
		...
		const_iterator(const_iterator&& right)						:ptr(right.ptr) { right.ptr = nullptr; }
		const_iterator& operator=(const const_iterator& right)	{ ptr = right.ptr; return *this; }
		const_iterator& operator=(const_iterator right)			{ ptr = right.ptr; right.ptr = nullptr; return *this; }
		~const_iterator() {}
	...
	public:
		...
		reverse_iterator(reverse_iterator&& right)		:ptr(right.ptr) { right.ptr = nullptr; }
		reverse_iterator& operator=(const reverse_iterator& right)	{ ptr = right.ptr; return *this; }
		reverse_iterator& operator=(reverse_iterator right)			{ ptr = right.ptr; right.ptr = nullptr; return *this; }
		~reverse_iterator() {}
	...
	public:
		...
		const_reverse_iterator(const_reverse_iterator&& right)			:ptr(right.ptr) { right.ptr = nullptr; }
		const_reverse_iterator& operator=(const const_reverse_iterator& right)	{ ptr = right.ptr; return *this; }
		const_reverse_iterator& operator=(const_reverse_iterator right)			{ ptr = right.ptr; right.ptr = nullptr; return *this; }
	...
	inline vector<T>::iterator vector<T>::emplace(const const_iterator& position, Types && ...args)
	{
		CheckValidIter(position);
	...
	template<typename T>
	template<class ...Types>
	inline vector<T>::iterator vector<T>::emplace_back(Types && ...args)
	{
		if (m_size == m_capacity)
			AllocateAndMove(CalculateNewCapacity());

		allocator::construct(m_data, m_size++, forward<Types>(args)... );

		return iterator(m_data+m_size-1);
	}

	template<typename T>
	inline vector<T>::iterator vector<T>::inset(const const_iterator& position, const T& value)
	{
		CheckValidIter(position);
	...
	template<typename T>
	inline vector<T>::iterator vector<T>::inset(const const_iterator& position, T&& value)
	{
		CheckValidIter(position);
	...
	template<typename T>
	inline vector<T>::iterator vector<T>::inset(const const_iterator& position, size_t count, const T& value)
	{
		CheckValidIter(position);
	...
	template<typename T>
	inline vector<T>::iterator vector<T>::erase(const const_iterator& position)
	{
		CheckValidIter(position);
	...
	template<typename T>
	inline vector<T>::iterator vector<T>::erase(const const_iterator& first, const const_iterator& last)
	{
		CheckValidIter(first);
		CheckValidIter(last);
	...
	template<typename T>
	inline void vector<T>::ShiftLeft(size_t count)
	{

	}

	template<typename T>
	inline void vector<T>::ShiftRight(size_t count)
	{
		if (m_size + count > m_capacity)
			AllocateAndMove(m_size + count);
		
	}

	template<typename T>
	inline void vector<T>::CheckValidIter(const_iterator& position)
	{
